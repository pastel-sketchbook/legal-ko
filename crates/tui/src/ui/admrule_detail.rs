...
fn render_detail_title(f: &mut Frame, app: &App, theme: &Theme, area: Rect) {
    ...
    // Right-side metadata
    if let Some(ref detail) = app.admrule_detail {
        let meta = format!(
            " {} · {} · {} ",
            detail.entry.rule_type, detail.entry.agency, detail.entry.date
        );
...
fn render_detail_content(f: &mut Frame, app: &App, theme: &Theme, area: Rect) {
    ...
    let lines = &app.admrule_detail_rendered_lines;
    if lines.is_empty() {
        return;
    }

    let visible_height = area.height as usize;
    let scroll = app.admrule_detail_scroll;
...
fn render_detail_footer(f: &mut Frame, app: &App, theme: &Theme, area: Rect) {
    let content = if let Some(ref msg) = app.status_message {
        ...
        let scroll_info = if app.admrule_detail_lines_count > 0 {
            let pct = if app.admrule_detail_lines_count <= 1 {
                100
            } else {
                (app.admrule_detail_scroll * 100) / app.admrule_detail_lines_count.saturating_sub(1)
            };
            format!(" {pct}% ")
        } else {
            String::new()
